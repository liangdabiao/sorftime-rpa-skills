"""sorftime 选品牌 scraper.

LIMITATION: The 选品牌 page (/home/choosebrand) is filter-gated,
same as 选关键词. The side-Keyword VM reuses the same infrastructure
but with brand-specific column schema. See SKILL.md for details.
...
from common import (
    ...
    ensure_brand_page, find_side_keyword_vm, read_state, write_csv,
)
...
def fetch_station(station_code, sleep_after=8.0):
    ...
    print(f"[{station_code}] navigate to brand page (site={site_id})...",
          file=sys.stderr)
    ensure_brand_page(session="sorftime-brand", site=site_id,
                      sleep_after=sleep_after)

    vm_check = find_side_keyword_vm("sorftime-brand")
    ...
    state = read_state("sorftime-brand")
...
def main():
    ...
    print(f"\nNOTE: 选品牌 page requires manual category selection per station.")
